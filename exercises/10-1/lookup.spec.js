...
const lookup = (login, property) => {
  const userLoginArr = users.map(user => user.login);
  const propertiesArr = ['login', 'firstName', 'lastName', 'likes'];
  if (!userLoginArr.includes(login)) {
    throw 'Could not find user';
  }
  if (!propertiesArr.includes(property)) {
    throw 'Could not find property';
  }
  return users.filter(name => name.login === login)
